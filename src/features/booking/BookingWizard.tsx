...
export function BookingWizard() {
  ...
  const selectedDaySlots: SlotsDay["slots"] = useMemo(() => {
    if (!selectedDate || !slotsData) return [];
    const d = slotsData.days.find((x) => x.date === selectedDate);
    return d?.slots ?? [];
  ...
  useEffect(() => {
    if (step === 2 && !slotsData && !slotsLoading && !slotsError) {
      void loadSlots();
    }
  ...
  const validateForm = (): boolean => {
    if (
      ...
      !form.company.trim() ||
      !form.sector.trim() ||
      !form.cityCountry.trim() ||
      !form.primaryNeed.trim()
    ) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-text-muted">{t.stepLabel(step)}</p>
        ...
        <div className="mt-10 space-y-6">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <label className="block sm:col-span-2">
              ...
              <input className={inputClass} value={form.sector} onChange={(e) => setForm((p) => ({ ...p, sector: e.target.value }))} />
            </label>
            <label className="block sm:col-span-2">
              <span className="text-sm font-medium text-text-primary">{t.fields.cityCountry} *</span>
              <input
                className={inputClass}
          ...
          <div className="flex justify-end pt-2">
            <Button type="button" variant="primary" onClick={goStep2}>
              {t.next}
            </Button>
          </div>
        </div>
      ...
      {step === 2 ? (
        <div className="mt-10 space-y-6">
          ...
          {!slotsLoading && slotsData && daysWithSlots.length > 0 ? (
            <div className="grid gap-8 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)] lg:items-start">
              ...
              <div className="rounded-2xl border border-border-subtle bg-bg-panel p-4 sm:p-5">
                {!selectedDate ? (
                  <p className="text-sm text-text-muted">{t.selectDayFirst}</p>
                ) : (
                  <>
                    <h3 className="text-sm font-semibold text-text-primary">{t.timesPanelTitle}</h3>
                    <p className="mt-1 text-xs capitalize text-text-muted">{formatDayCard(selectedDate, locale as LocaleCode)}</p>
          ...
          <div className="flex flex-wrap justify-between gap-3 pt-4">
            ...
            <Button type="button" variant="primary" disabled={!selectedSlotStart || slotsLoading} onClick={goConfirm}>
              {t.next}
            </Button>
          </div>
        </div>
        ...
        <div className="mt-10 space-y-6">
          ...
          <div className="rounded-2xl border border-border-subtle bg-bg-panel p-5 text-sm leading-relaxed text-text-muted">
            ...
            <ul className="mt-3 list-inside list-disc space-y-1">
              ...
              <li>
                {form.company} / {form.sector} / {form.cityCountry}
              </li>
              <li>{form.primaryNeed}</li>
              {selectedSlotStart ? (
                <li>
